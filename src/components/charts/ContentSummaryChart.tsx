...
import { useSummaryByContent } from '@/hooks/useSummaryByContent'
import { useFilters } from '@/contexts/FilterContext'
import { Badge } from '@/components/ui/badge'

const COLORS = ['#3B82F6','#10B981','#F59E0B','#EF4444','#8B5CF6','#06B6D4','#84CC16','#F97316','#22C55E','#EAB308']

export function ContentSummaryChart() {
  const { data, isLoading, error } = useSummaryByContent()
    ...
    return (
      <div className="h-80 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-2">Erro ao carregar resumo por conteúdo</p>
  ...
  return (
    <div className="w-full">
      <div style={{ height: processed.chartHeight }}>
        <ResponsiveContainer width="100%" height="100%">
            ...
            const ClickableCursor = (props: any) => {
              ...
              return (
                <rect
                  ...
                  fill={'hsl(var(--muted))'}
                  opacity={0.12}
                  onClick={() => { if (name) setCategoricalFilter('content', name) }}
                />
              )
            }
            return (
              <Tooltip 
                content={<CustomUTMTooltip total={processed.total} title="Conteúdo" />}
          ...
          <Bar dataKey="value" radius={[0, 4, 4, 0]} onClick={(data) => {
            ...
            if (name) setCategoricalFilter('content', name)
          }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mt-4 w-full overflow-x-hidden">
        ...
        <div className="rounded-xl bg-muted/40 border border-border/50 p-4">
          <p className="text-xs text-muted-foreground mb-2">Principal Conteúdo</p>
