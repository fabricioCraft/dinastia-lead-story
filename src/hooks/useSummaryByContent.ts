...
export function useSummaryByContent() {
  ...
  return useQuery<SummaryItem[]>({
    queryKey: ['summary-by-content', days, startDate, endDate, cf],
    queryFn: async () => {
      ...
      const url = `/api/dashboard/summary-by-content${params.toString() ? `?${params.toString()}` : ''}`
      const res = await fetch(url)

      if (!res.ok) throw new Error(`Erro ao buscar resumo por conteúdo: ${res.status}`)
